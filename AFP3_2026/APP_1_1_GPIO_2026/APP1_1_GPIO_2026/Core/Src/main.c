...
int main(void)
{
  ...
  MX_USART3_UART_Init();
  /* USER CODE BEGIN 2 */
MX_GPIO_Init();           // Inicialización desde la API
MX_USART3_UART_Init();
  ...
  /* USER CODE BEGIN WHILE */
	  while (1)
	  {
	     for (uint8_t i = 0; i < 3; i++)
	        {
	            writeLedOn_GPIO(LEDS[i]); // Enciende LED
	            HAL_Delay(200);  // Espera 200ms
	            writeLedOff_GPIO(LEDS[i]); // Apaga LED
	            HAL_Delay(200); // Espera 200ms antes del siguiente
	        }
